...
void MainWindow::listSelectionChanged(const QItemSelection selected, const QItemSelection deselected)
{
    ...
    if (items.count() == 1) {
        ...
        statusDate->setText(QString("%1").arg(QLocale().toString(file.lastModified().date(), QLocale::ShortFormat)));
    }
    else {
  ...
  if (focusWidget() == list || focusWidget() == detailTree) {
    ...
    if (listSelectionModel->hasSelection()) {
      ...
      if (!curIndex.isDir()) {
        ...
        // Add menus
        // TODO: ???
        QMultiHashIterator<QString, QMenu*> m(*customActManager->getMenus());
        while (m.hasNext()) {
          m.next();
